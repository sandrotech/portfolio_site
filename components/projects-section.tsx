...
function ProjectVisual({ project }: { project: Project }) {
  ...
  return <div role="img" aria-label={project.imageAlt} className="relative aspect-[16/10] overflow-hidden bg-[radial-gradient(circle_at_80%_15%,rgba(14,175,164,0.16),transparent_28%),linear-gradient(145deg,#DFEBF8,#F8FBFF)] p-5"><div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(16,42,76,0.04)_1px,transparent_1px),linear-gradient(to_bottom,rgba(16,42,76,0.04)_1px,transparent_1px)] bg-[size:20px_20px]" /><div className="relative flex h-full gap-3 rounded-xl border border-border bg-card/95 p-4 shadow-lg"><div className="flex w-12 shrink-0 flex-col items-center gap-3 rounded-lg border border-border bg-background-alt py-3 text-primary"><Icon className="h-5 w-5 text-accent" aria-hidden="true" /><Workflow className="h-4 w-4 text-primary/60" aria-hidden="true" /><LayoutDashboard className="h-4 w-4 text-primary/60" aria-hidden="true" /></div><div className="flex flex-1 flex-col gap-3"><span className="h-3 w-2/3 rounded-full bg-primary/20" /><span className="h-2 w-full rounded-full bg-border" /><div className="grid flex-1 grid-cols-2 gap-2"><span className="rounded-md bg-card-highlight" /><span className="rounded-md border border-border bg-background" /></div><span className="h-2 w-4/5 rounded-full bg-accent/20" /></div></div></div>
}
